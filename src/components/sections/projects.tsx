              ...
              {project.link && (
                <Button variant="outline" size="sm" asChild>
                  <a href={project.link} target="_blank" rel="noopener noreferrer">
                    {translations.projects.visitDemo} <ArrowUpRight className="ml-2 h-4 w-4" />
                  </a>
                </Button>
              )}
              {project.apk && (
                   <Button variant="default" size="sm" asChild>
                      <a href={project.apk} download>
                          {translations.projects.downloadApk} <Download className="ml-2 h-4 w-4" />
                      </a>
                  </Button>
              )}
          </div>
...
export default function Projects({ projects }: ProjectsProps) {
  ...
  const getCategoryName = (category: string) => {
    return translations.projects.categories[category.replace(/ & /g, '_').toLowerCase() as keyof typeof translations.projects.categories] || category;
  }
  return (
